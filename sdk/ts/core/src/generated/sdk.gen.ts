...
/**
 * Delete a message by id, resolving its channel (soft)
 *
 * Deletes a message given only its id - the channel is resolved server-side. Authorisation mirrors the channel-scoped delete: a server key may delete any message; an end user must be a member of the resolved channel and either the author or a moderator/owner. Unknown id is a 404.
 */
export const deleteV1MessagesMessageid = <ThrowOnError extends boolean = false>(options: Options<DeleteV1MessagesMessageidData, ThrowOnError>): RequestResult<DeleteV1MessagesMessageidResponses, DeleteV1MessagesMessageidErrors, ThrowOnError> => (options.client ?? client).delete<DeleteV1MessagesMessageidResponses, DeleteV1MessagesMessageidErrors, ThrowOnError>({ url: '/v1/messages/{messageId}', ...options });
