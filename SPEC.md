...
| Drop-in `stream-chat` adapter (added 26/07/2026, downstream migration) | `@firemoot/stream-compat` exports a **`StreamChat`** class fusing both modes behind Stream's own constructor overloads (`(key, options)` = browser, `(key, secret, options)` = server-trusted), plus `getInstance()`. Uncovered methods exist and **throw** a named `FiremootCompatError`; exactly three are documented no-ops | Model-level compatibility (the §2 "mirror Stream's model" decision) still left every migrating app rewriting its adapter modules. A symbol-level adapter drops that to a config change - alias `stream-chat` in the bundler and application code is untouched - which is the difference between an evaluation that takes an afternoon and one that takes a sprint. Fusing both modes into one class is not a stylistic choice: `stream-chat` *is* one class, so any split would break the alias. Two consequences are load-bearing. **`createToken` stays synchronous** (Stream's returns a `string`, and app code assigns it inline), which rules out WebCrypto's promise-only `subtle` and, because a browser bundle must not fail to resolve `node:crypto`, is served by a ~100-line in-package HMAC-SHA256 differential-tested against `node:crypto`. It mints Firemoot's claims (`sub` + a *required* `exp`, defaulting to 1h) rather than Stream's non-expiring `user_id` token - the one behaviour change a migrating app must plan for. **Loud failure over silent no-ops**: a stubbed-out `mute()` would let a migration look complete while moderation quietly did nothing, so every uncovered method throws with its own name and a link. The three exceptions (`notification.message_new`/`notification.mark_unread` subscriptions, `channel.sendEvent()`) are ported from the downstream facade layer, where real call sites make them harmlessly. `verifyWebhook` accepts both header encodings and keys off the **endpoint** secret via `firemoot.webhookSecret` |
...
firemoot/
  ...
  sdk/ts/          # @firemoot/client (state layer) + @firemoot/core (generated transport)
                   # + @firemoot/stream-compat (the stream-chat API surface)
...
- **`@firemoot/stream-compat`** - the getstream.io `stream-chat` API surface over
  `@firemoot/client`: a `StreamChat` class fusing the browser and server-trusted
  modes behind Stream's own constructors, so a migrating app changes configuration
  rather than code.
